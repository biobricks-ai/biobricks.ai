...
title: clinvar
namespace: clinvar
description: ClinVar aggregates information about genomic variation and its relationship
  to human health.
dependencies:
- name: clinvar
...
#### Data Table List:
 - ``data/ACMG_gene_interp_history.parquet``  
 - ``data/allele_gene.txt.parquet`` 
 - ``data/combined_variant_summary_xab.txt.parquet`` 
 - ``data/combined_variant_summary_xac.txt.parquet`` 
 - ``data/gene_specific_summary.parquet`` 
 - ``data/organization_summary.parquet`` 
 - ``data/variation_allele.txt.parquet``
 - ``data/HGVS_summary.txt.parquet`` 
 - ``data/clinical_significance_updates.parquet`` 
 - ``data/hgvs4variation.txt.parquet``
 - ``data/submission_summary.txt.parquet``
 - ``data/cross_references.parquet``  
 - ``data/molecular_consequences.parquet`` 
 - ``data/var_citations.parquet``

### gene_specific_summary
 * **Overview** Shows the gene list,the number of submissions and the different alleles
